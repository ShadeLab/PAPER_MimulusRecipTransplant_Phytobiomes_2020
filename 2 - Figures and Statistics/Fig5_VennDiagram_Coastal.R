...
mim_envir<-rownames(subset(mim_envir, Site_planted =="Coastal"))
mim_envir 
...
mim_envir 
Coastalrhizo_coastaleco <- subset(mim_otu, select=colnames(mim_otu) %in% mim_envir)
#determine present OTUs
Coastalrhizo_coastaleco$Sums <-rowSums(Coastalrhizo_coastaleco)
coastaleco<-rownames(subset(Coastalrhizo_coastaleco, Sums >= 1))
#this is 9263 OTUs.
...
mim_envir 
Coastalrhizo_inlandeco <- subset(mim_otu, select=colnames(mim_otu) %in% mim_envir)
#determine present OTUs
Coastalrhizo_inlandeco$Sums <-rowSums(Coastalrhizo_inlandeco)
inlandeco<-rownames(subset(Coastalrhizo_inlandeco, Sums >= 1))
#this is 9245 OTUs.
...
mim_envir 
Coastalbulk <- subset(mim_otu, select=colnames(mim_otu) %in% mim_envir)
#determine present OTUs
Coastalbulk$Sums <-rowSums(Coastalbulk)
bulk<-rownames(subset(Coastalbulk, Sums >= 1))
#this is 7933 OTUs.
...
#Verify total number of OTUs in or intersecting with CoastalEco. Should be 9263.
...
#Verify total number of OTUs in or intersecting with InlandEco. Should be 9245
...
#Verify total number of OTUs in or intersecting with Bulk. Should be 7933.
...
v <-draw.triple.venn(area1 = length(attr(ItemsList,"intersections")$CoastalEco)+length(attr(ItemsList,"intersections")$'CoastalEco:Bulk')+length(attr(ItemsList,"intersections")$'CoastalEco:InlandEco')+length(attr(ItemsList,"intersections")$'CoastalEco:InlandEco:Bulk'), 
                    ...
                    n13 = length(attr(ItemsList,"intersections")$'CoastalEco:Bulk')+length(attr(ItemsList,"intersections")$'CoastalEco:InlandEco:Bulk'),  
                    n123 = length(attr(ItemsList,"intersections")$'CoastalEco:InlandEco:Bulk'), 
                    category = c("Coastal ecotype", "Inland ecotype", "Bulk soil"), cat.cex=rep(3,3),lty = "blank",     fill = c("blue", "red", "green"),cex=rep(3,7),cat.just =list(c(0.25, 1), c(0.75, 1), c(0.5, 0)))
...
lapply(v, function(i) i$label)
library(VennDiagram)
test<-tiff("Figure5.tiff", width=6000,height=6000, units="px",res=300)
grid.newpage()
grid.draw(v)
dev.off()

#could not figure out how to make width/height smaller (while keeping res=300) without
#the text looking enormous. The workaround is to resize the image elsewhere.
